...
public fun TextProposal.toAny(): Any = Any(TextProposal.TYPE_URL, with(TextProposalConverter) {
    toByteArray() })

public fun Any.parse(converter: ProtobufConverter<TextProposal>): TextProposal {
  if (typeUrl != TextProposal.TYPE_URL) throw IllegalStateException("Please check the type_url")
  return value.parseProtobuf(converter)
}
