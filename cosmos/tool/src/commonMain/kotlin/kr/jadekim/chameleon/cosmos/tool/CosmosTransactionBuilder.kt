...
import cosmos.crypto.secp256k1.PubKeyConverter
import cosmos.crypto.secp256k1.parse
...
class CosmosTransactionBuilder {
    ...
    fun signer(vararg signerInfo: SignerInfo): CosmosTransactionBuilder {
        if (signerInfo.isNotEmpty() && fee.payer.isBlank()) {
            val payer = CosmosWallet(signerInfo.first().publicKey.parse(PubKeyConverter).key).address.text
            val granter = fee.granter.ifBlank { payer }
            this.fee = fee.copy(payer = payer, granter = granter)
        }
