package kr.jadekim.chameleon.core.hd.secp256r1

import kr.jadekim.chameleon.core.crypto.secp256r1.Secp256r1
...
open class HDSecp256r1PublicKey(bytes: ByteArray) : PublicKey {

    val compressed: ByteArray =
        if (isCompressed(bytes)) bytes.copyOf()
        else if (isUncompressed(bytes)) Secp256r1.compressPublicKey(bytes)
        else throw IllegalArgumentException("Invalid public key")

    val uncompressed: ByteArray by lazy {
        if (isCompressed(bytes)) Secp256r1.decompressPublicKey(bytes)
    ...
    companion object {
        ...
        fun recovery(signature: ByteArray, message: ByteArray, recoveryId: Int): HDSecp256r1PublicKey {
            return HDSecp256r1PublicKey(Secp256r1.recoveryPublicKey(signature, message, recoveryId))
        }
         ...
         */
        fun recovery(signature: ByteArray, message: ByteArray): Pair<HDSecp256r1PublicKey, HDSecp256r1PublicKey> {
            val key1 = HDSecp256r1PublicKey(Secp256r1.recoveryPublicKey(signature, message, 0))
            val key2 = HDSecp256r1PublicKey(Secp256r1.recoveryPublicKey(signature, message, 1))

            return key1 to key2
        }
    }

    operator fun plus(that: HDSecp256r1PublicKey): HDSecp256r1PublicKey = HDSecp256r1PublicKey(
        Secp256r1.combinePublicKey(arrayOf(compressed, that.compressed))
    )

    operator fun unaryMinus(): HDSecp256r1PublicKey = HDSecp256r1PublicKey(Secp256r1.publicKeyNegate(compressed))

    operator fun minus(that: HDSecp256r1PublicKey): HDSecp256r1PublicKey = this + -that

    operator fun times(that: HDSecp256r1PublicKey): HDSecp256r1PublicKey =
        HDSecp256r1PublicKey(Secp256r1.publicKeyTweakMul(compressed, that.compressed))

    override fun verify(message: ByteArray, signature: ByteArray): Boolean =
        Secp256r1.verify(signature, message, compressed)

    fun isValid(): Boolean = Secp256r1.isValidPublicKey(compressed)
