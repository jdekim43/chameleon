...
import kr.jadekim.chameleon.core.privateKeyFromBase58
import kr.jadekim.chameleon.core.publicKeyFromBase58
import kr.jadekim.chameleon.core.toBase58PrivateKey
import kr.jadekim.chameleon.core.toBase58PublicKey
    ...
    Bip32TestData.load().forEach { item ->
        ...
        context("seed = ${item.seed}") {
            item.keys.forEach { key ->
                ...
                context(key.path) {
                    test("private key") {
                        childrenPrivateKey.toBase58PrivateKey(VERSION_MAINNET_PRIVATE) shouldBe key.extendedPrivateKey
                    }
                    test("public key") {
                        @Suppress("UNCHECKED_CAST")
                        (childrenPrivateKey.extendedPublicKey as ExtendedKey<HDSecp256k1PublicKey>)
                            .toBase58PublicKey(VERSION_MAINNET_PUBLIC) shouldBe key.extendedPublicKey
                    }
                }
            }
        }
